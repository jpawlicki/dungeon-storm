abilityData.BLOODLUST = new class extends Ability {
	constructor() {
		...
		this.details = ["!REACTION: Gain ♦ whenever a !ENEMY becomes !BLOODY."];
		this.aiHints = [];
	...
	actionEvent(unit, action) {
		let reactions = [];
		let newBloodied = 0;
		for (let e of action.effects) if (e.property == "state" && e.unit.player != unit.player && e.value == Unit.State.BLOODIED && e.oldValue != Unit.State.BLOODIED) newBloodied++;
		if (newBloodied > 0) {
